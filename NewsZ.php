...
# Modifications to News.php Copyright (C) 2012 Daniel Yount <factorf2@yahoo.com>
#
...
$wgExtensionCredits['parserhook'][] = array(
	...
	'author' => 'Fabio Zorba , Daniel Yount - factorf2@yahoo.com',
		...
		if ( ! $newsDateAndPreview ) {
			if ($image_name == false )
				#$logo='[[Image:Logo-' . $article_cat . '.png|center|' . $thumbsize .'|link=' . $row->page_title . ']]';
                                $logo='<A HREF="'.$row->page_title.'" ><IMG SRC="http://www.userspace.org/images/Logo-'.$row->page_title.'.png" ></A>';
		...
		if ($newscols > 15) {
			...
			if ( ! $newsDateAndPreview ) {
				...
				//'" target="_blank">' . $content . '</a>&nbsp;' . $timestamp . '</td>';
                                $output .= '<td style="vertical-align:top">' . $logo .'</td>' .
				'<td align="left" valign="top" style="vertical-align:top; text-align:left"><B><H3>' . $content . '</H3></td>';
			}
			else {
			...
			if ( $newscols == 0 ) {
				...
				//$output .= '<td align="left" valign="center" style="text-align:left">' . $timestamp;
                                $output .= '<td align="left" valign="center" style="text-align:left">';
				$output .= ' - <a href="' . fzURLencode ($row->page_title) .'" target="_blank">'
				. $content . '</a>';

			}
			else $output .= '<td style="text-align:center">' . $logo . ' ' . $content . '</td>';
		}
